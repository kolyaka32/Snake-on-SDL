...
#define SCREEN_WIDTH gridX*CELL_SIDE
#define SCREEN_HEIGHT gridY*CELL_SIDE
...
int Head::move(){
    ...
    if(y*gridX + x == Apple.getPos()){
        ...
        TileArray[position].set(dest.x / CELL_SIDE, dest.y / CELL_SIDE, vec, newVec);
        fat = 1;
        dest.x = x * CELL_SIDE; dest.y = y * CELL_SIDE;
        vec = newVec;
        if(length >= maxLength){
            winning = true;
        }
